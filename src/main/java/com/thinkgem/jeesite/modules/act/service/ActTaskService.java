...
import com.thinkgem.jeesite.common.mapper.insertRollBack.RollBack;
...
public class ActTaskService extends BaseService {
     ...
     * @param sign       待办任务和待办报销区分
     * @return
     */
    public List<Act> todoList(Act act, String loginName, String title, String sign) {
        ...
        if (sign == null) {
            ...
            /**新流程*/
            get.add ( "thd_getsale" );
            todoTaskQuery.processDefinitionKeyIn ( get );
        }
        ...
        List<String> getSalelist = getSaleDao.selectProcInsId ();//用户撤回的数据ID
        List<String> contractlist = contractDao.selectProcInsId ();//用户撤回的数据ID(合同)
        for (Task task : todoList) {
            if (task.getName ().contains ( "修改" )) {
                continue;
            }
            if (signbool) {
                if (task.getProcessDefinitionId ().contains ( "getSale" )
                        || task.getProcessDefinitionId ().contains ( "thd_getsale" )) {
                    continue;
                }
            }

            Boolean bool = true;
            for (String procInsId : getSalelist) {
                if (procInsId != null && task.getProcessInstanceId ().equals ( procInsId )) {
                    bool = false;
                }
            }
            for (String procInsId : contractlist) {
                if (procInsId != null && task.getProcessInstanceId ().equals ( procInsId )) {
                    bool = false;
                }
            }
            if (bool) {
                ...
                e.setStatus ( "todo" );
                if (loginName != null) {
                    if (!loginName.equals ( e.getVars ().getMap ().get ( "apply" ) )) {
                        continue;
                    }
                }
                if (StringUtils.isNotBlank ( title )) {
                    String tit = (String) e.getVars ().getMap ().get ( "title" );
                    if (tit.indexOf ( title ) == -1) {
                        continue;
                    }
                }
                result.add ( e );
        ...
        TaskQuery toClaimQuery = taskService.createTaskQuery ().taskCandidateUser ( userId )
                .includeProcessVariables ().active ().orderByTaskCreateTime ().asc ();
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
    ...
    public Page<Act> historicList(Page<Act> page, Act act, String loginName, String title) {
        String userId = UserUtils.getUser ().getLoginName ();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//		List<HistoricTaskInstance> histList = histTaskQuery.listPage(page.getFirstResult(), page.getMaxResults());
        ...
        int count = 0;
        List<String> gsList = getSaleDao.selectProcInsId ();//用户撤回的数据ID
        List<String> ll = rollBack.selectRollBack ();//用户回退的taskId
        for (HistoricTaskInstance histTask : histList) {
            ...
            for (int i = 0; i < gsList.size (); i++) {
                if (histTask.getProcessInstanceId ().equals ( gsList.get ( i ) )) {
                    gsList.remove ( i );
                    bool = false;
                    continue;
                }
            ...
            for (int i = 0; i < ll.size (); i++) {
                if (histTask.getId ().equals ( ll.get ( i ) )) {
                    bool = false;
                    continue;
                }
            }
            ...
            count++;
            if ((no - 1) * size < count && count <= no * size) {
                actList.add ( e );
            }
        }
        page.setCount ( count );
     ...
     * @param page
     * @param procDefKey 流程定义标识
     * @param loginName  用户搜索条件
     * @return
     */
    public Page<Act> endList(Page<Act> page, Act act, String loginName, String title) {
...
//						e.setComment(commentList.get(0).getFullMessage());
                        CommentEntity commentEntity = (CommentEntity) commentList.get ( 0 );
                        e.setComment ( commentEntity.getMessage () );
                    }
                }
                actList.add ( e );
            }
