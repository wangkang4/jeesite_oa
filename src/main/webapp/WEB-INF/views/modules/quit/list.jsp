...
<body>
	<ul class="nav nav-tabs" id="list">
		<li class="active"><a href="${ctx}/tb/quit/list">离职申请列表</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="quit">
			<tr>
				...
				<c:choose>
					<c:when test="${quit.statu=='驳回'}">
	                    <a href="${ctx}/tb/quit/add?id=${quit.id}">详情</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					<c:when test="${quit.proneText!=null || quit.prtwoText!=null || quit.prthreeText!=null || quit.prfourText!=null}">
	                   	<c:choose>
							<c:when test="${quit.statu=='驳回'}">
			                   	<a href="${ctx}/tb/quit/back?id=${quit.id}">销毁申请</a>
							</c:when>
							<c:otherwise>
								不可撤销
							</c:otherwise> 
