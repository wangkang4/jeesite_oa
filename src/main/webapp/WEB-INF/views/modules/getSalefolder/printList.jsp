	...
	<div class="table" id="sale" style="display:block">
		<div class="header">
			<div>
				...
				<span class="font_three">您好,高会敏</span>
			</div>
			
		</div>
        <div class="top">
            	（${listall[0].costType }）费用报销单
        </div>
        <div class="two">
            <span class="name" style="text-indent:1rem;text-align:left;">单位：北京桃花岛信息技术有限公司</span>
            <span><fmt:formatDate value="${listall[0].createDate}" pattern="yyyy年MM月dd日"/></span>
            <span>第${fn:substring(listall[0].reason,0,10)}号</span>
        </div>
        <table border="0" cellspacing="0">
		  ...
		  <c:forEach items="${listall[1]}" var="listOther">
			<tr>
				<td class="proc"><fmt:formatDate value="${listOther.detailDate}" pattern="yyyy-MM-dd"/></td>
				<td class="proc">${listOther.costDescription}</td>
				<td class="proc">${listOther.amountMoney}元</td>
				<td class="proc">${listOther.projectName}</td>
				<td class="proc">${listOther.information}</td>
			</tr>
		  </c:forEach>
          <tr>
              ...
              <td colspan="2" style="text-align:left;text-indent:2rem;">
				<fmt:formatNumber type="number" value="${listall[0].forMoney}" maxFractionDigits="2" pattern="#.00"/>元
              </td>
          </tr>
          <tr>
              <td class="black">
              ...
              <td style="text-align:left;text-indent:2rem;" colspan="1">${listall[0].user.name}</td>
              <td class="black">
                  <span>所属部门</span>
              </td>
              <td style="text-align:left;text-indent:2rem;" colspan="2">${listall[0].office.name}</td>
          </tr>
          <tr>
          ...
          <c:forEach items="${listall[3]}" var="hlist">
					<tr>
