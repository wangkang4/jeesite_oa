...
<body>
	<ul class="nav nav-tabs" id="list">
		<li class="active"><a href="${ctx}/tb/travelApply/list">出差申请列表</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="travelApply">
			<tr>
				...
				<td>
				<c:choose>
					<c:when test="${travelApply.statu=='驳回'}">
	                    <a href="${ctx}/tb/travelApply/add?id=${travelApply.id}">详情</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					<c:when test="${travelApply.proneText!=null || travelApply.prtwoText!=null }">
	                   	<c:choose>
							<c:when test="${travelApply.statu=='驳回'}">
			                   	<a href="${ctx}/tb/travelApply/back?id=${travelApply.id}">销毁申请</a>
							</c:when>
							<c:otherwise>
								不可撤销
							</c:otherwise> 
