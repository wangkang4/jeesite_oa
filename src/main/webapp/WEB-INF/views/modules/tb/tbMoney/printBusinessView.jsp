...
#Business table .high td:nth-child(2){
	text-align: left;
	padding: 7px 8px;
}
...
<body>
<!--startprint-->
	<div class="table" id="Business" style="display:block">
		<div class="header">
			<div>
        ...
        <table border="0" cellspacing="0">
		  ...
		  <tr>
		  	<td class="gray">用餐地点</td>
		  	<td>${business.place }</td>
		  	<td class="gray">用餐金额</td>
		  	<td><fmt:formatNumber type="number" value="${business.money }" maxFractionDigits="2" pattern="#0.00"/>元</td>
		  </tr>
		  <c:forEach items="${list }" var="rs">
					<c:if test="${rs.type==0 }">
						<tr>
							<td class="gray">被招待人员</td>
							<td>${rs.name }</td>
							<td class="gray">被招待人员职位</td>
							<td>${rs.position }</td>
						</tr>
					</c:if>
				</c:forEach>
		  <tr class="high">
		  	<td class="gray">备注</td>
		  	<td colspan="3">${business.notes }</td>
		  </tr>
          <tr>
